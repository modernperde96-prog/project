      ...
      <section className="py-24">
        <div className="container mx-auto max-w-xl px-4">
          <div className="rounded-[2rem] border border-border bg-card p-8 shadow-sm md:p-10">
            ...
            <h1 className="mt-3 text-3xl font-serif font-bold">{dict.pages.auth.loginTitle}</h1>
            <p className="mt-3 text-muted-foreground">{dict.pages.auth.loginDescription}</p>
            <form className="mt-8 grid gap-4">
              ...
              <button className="h-12 rounded-full bg-primary px-6 font-semibold text-primary-foreground">{dict.pages.auth.actionLogin}</button>
            </form>
            <p className="mt-6 text-sm text-muted-foreground">
              <Link href={localizePath(currentLocale, '/signup')} className="text-primary hover:underline">{dict.pages.auth.switchToSignup}</Link>
            </p>
          </div>
        </div>
      </section>
